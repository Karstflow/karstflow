...
| `paradencer-storage` | `partial` | Hot-state, snapshot catalog, checksum/invariant checks, retention + runtime-state apply/rollback store for execution effects (`RuntimeStateStore`) with typed receipts/snapshots and strict monotonic + rollback-order guards + checked counter-overflow detection (`StateCounterOverflow`) for hot-state/runtime-state accumulators (no silent saturating wrap) + atomic apply semantics on overflow paths (no partial state mutation before overflow rejection) + checked `usize -> u64` counter conversions in hot-state/runtime-state paths (no direct cast-based truncation risk on wider-pointer targets) + **account database with funk-inspired transaction/fork model** (`AccountDatabase` with versioned records, copy-on-write semantics, publish/cancel transaction lifecycle, concurrent access via DashMap) + **account primitives** (custom `Pubkey`, `Account`, `AccountMeta`, `AccountData` without solana-sdk dependencies) + **transaction types** (`Transaction`, `Instruction`, `Signature`, `AccountRef` with read/write mode tracking) + **transaction processor** (`TransactionProcessor` with account loading, simulated execution, writeback to database, batch processing support) + **persistent storage** (`DurableStore` trait with `FileDurableStore` sled backend, column families, batch writes, prefix/range scan) + **account persistence** (disk fallback reads, publish-time persist, bulk insert, binary encode/decode, startup recovery with stats) + **blockstore persistence** (persistent backend variant, root recovery on startup, all operations dispatch to durable store) + **StorageEngine facade** (unified open/recover/compact/flush/disk_usage, blockstore path management) + **compaction** (slot-keyed record cleanup across 8 column families) + **snapshot bank state manifest parser** (custom BincodeReader for Solana snapshot manifest, SnapshotBankState with epoch/rent/inflation/fee/stakes config, blockhash queue, hard forks) | Real SVM/sBPF execution integration, full accounts state lifecycle with rent/compute budget enforcement, catch-up/restore pipeline depth, snapshot network download/verification, status cache parsing | Critical |
| `paradencer-stages` | `partial` | Intake/filter/assembler/metrics + replay/fork-choice scaffolding + class-aware retry budget enforcement in block assembly + replay candidate stale-pruning policy + fork-choice switch hysteresis + confirmed-only reorg hold option + failed-ratio-gated candidate confirmation + runtime-like execution policy path wired to storage-backed adapter by default + runtime-like channel apply policies (`strict/lenient`) plumbed from storage policy to adapter + execution error-path telemetry + configurable execution error handling mode (`fail_open` / `fail_fast`) with stage-level integration coverage via injected execution bridge + configurable fail-open circuit-breaker (`execution_error_fail_open_max_consecutive`) + **verify_stage** (batch Ed25519 signature verification pipeline with round-robin distribution, gossip vote routing) + **resolv_stage** (blockhash ring for transaction age validation, LRU stash for unknown blockhashes, automatic unstash) + **pack_stage** (priority-fee transaction scheduler with account write-lock conflict detection, microblock production, vote-first scheduling, per-block cost limits) + **exec_stage** (leader-mode execution tile with pluggable engine, Merkle entry hashing, per-microblock results) + **shred_network** (FEC set completion tracking from turbine/repair, duplicate detection, retransmit scheduling, slot pruning) | Deeper tile integration (pack↔exec↔PoH wiring), repair protocol integration, full turbine retransmit tree, snapshot restore tile orchestration | Critical |
...
- ✓ DurableStore persistent backend with sled (column families, batch writes, recovery).
- ✓ AccountDatabase disk persistence (fallback reads, publish persist, startup recovery).
- ✓ Blockstore persistence with root recovery.
- ✓ StorageEngine facade with compaction.
- ✓ Snapshot bank state manifest parser.
- Status cache parsing from snapshot.
- Large-state operations at scale.

5. Snapshot lifecycle parity:
- ✓ Snapshot archive reader + AppendVec parser + restore pipeline.
- ✓ Bank state manifest parser.
...
## Approximate Remaining Work Split (from current ~60% parity, 2026-02-18)

Remaining ~40% of logic to reach full Firedancer parity, broken by area:

- VM + Syscalls full fidelity: ~10% (remaining: full CPI privilege checks, JIT compiler, remaining SBPF edge cases)
- Storage + Snapshot restore pipeline: ~5% (status cache, snapshot network download, full genesis bootstrap)
- Tile pipeline (pack, net, metrics, full replay): ~8% (pack scheduling, net tile, metrics)
- Network protocol behavior: ~5% (gossip/repair fidelity, deeper QUIC)
- Types completeness: ~4% (all protocol message types)
- Consensus deepening: ~3% (optimistic confirmation, heavy fork)
- App/IPC/util protocol-specific: ~3% (tango IPC, workspace utilities)
- Crypto (BLS, ZK SDK): ~2%
- Final hardening + perf + rollout: ~2%
...
|---|---:|---:|---:|
| Runtime core (bank, executor, epoch processing) | 15% | 75% | 11.3% |
| Builtin programs (system, vote, stake, BPF loader, etc.) | 15% | 82% | 12.3% |
| Consensus / voting / tower / fork-choice | 12% | 70% | 8.4% |
| Execution runtime (sBPF VM + syscalls) | 10% | 80% | 8.0% |
| Rewards / stakes / sysvars | 8% | 83% | 6.6% |
| Cryptography (ed25519, blake3, bn254, reed-solomon) | 8% | 50% | 4.0% |
| Tile pipeline (replay, pack, shred, PoH, net) | 12% | 30% | 3.6% |
| Storage / accounts (accdb, funk, vinyl, snapshots) | 8% | 67% | 5.4% |
| Network stack (QUIC, TLS, HTTP, gossip, repair) | 6% | 30% | 1.8% |
| App / config / IPC / observability | 6% | 25% | 1.5% |

Summary:
- Total migrated logic estimate: **~62.9%** (rounded tracking value: **~63%**).
- Deep analysis performed 2026-02-17 comparing all Firedancer C modules line-by-line against Paradencer crates.
- Updated 2026-02-18: Storage 25% → 50% after fork tree, snapshot format, owner index, FEC tracker, restore pipeline.
- Updated 2026-02-18: Storage 50% → 60% after full bootstrap pipeline (lthash, stakes, stake history, sysvars, features, genesis bootstrap, bank hash verification, accounts hash, dirty-set tracking).
- This table is intentionally conservative and should be revised after each substantial subsystem move.

### Top gaps by impact (Critical path to working validator):
1. **VM + Syscalls** (80%) — core interpreter, syscalls, ELF loader, memory model, feature gates done. Remaining: full CPI privilege checks, JIT.
2. **Storage** (67%) — persistent backend + recovery + compaction + full bootstrap pipeline + Solana-compatible snapshot I/O (full + incremental) + manifest serializer (bank state + AccountsDbFields) done. Remaining: snapshot network download, WAL for crash recovery, account index persistence.
3. **Tile Pipeline** (30%) — pack tile, full net tile, metrics pipeline incomplete.
4. **Network behavior** (30%) — gossip/repair protocol fidelity, deeper QUIC integration.
5. **Types** (55%) — many secondary bincode types still missing for full protocol compat.

Deep analysis rebase (2026-02-17):
- Performed full line-by-line comparison of ALL Firedancer native modules against Paradencer crates.
- Firedancer native total recounted: ~623,600 lines C+H (13 subsystems, excluding discoh/Frankendancer).
- Previously uncounted modules added: waltz (56K), discof (56K), vinyl (17K), tango (9K), groove (3K).
- Weight table completely rebuilt with accurate per-module percentages.
- Overall parity re-estimated: **~37% → ~57%** (reflects more accurate analysis, not just new code).
- Created comprehensive Firedancer→Paradencer module mapping in `docs/90_module_mapping_working.md`.

Storage bootstrap cycles 15-19 (2026-02-18):
- Storage: 50% → 60% (9 cycles total: cycles 11-19)
  - Cycle 15: Feature set initialization from snapshot accounts (scan Feature-program-owned accounts, parse bincode Option<Slot>)
  - Cycle 16: SHA-256 Merkle accounts hash (16-way fanout, per-account SHA-256, sorted by pubkey)
  - Cycle 17: Genesis bootstrap pipeline (bootstrap_from_genesis: 8-step init, reuses all snapshot helpers)
  - Cycle 18: Bank hash verification after snapshot bootstrap (compare computed vs expected hash from manifest)
  - Cycle 19: Incremental dirty-set tracking (per-slot HashSet<Pubkey> on all write paths, drain API for incremental snapshots)
- Total new tests: +39 (712 consensus, 335 storage)
- See: `docs/plans/storage_bootstrap_cycles_15_19.md`

Storage snapshot cycles 20-23 (2026-02-18):
- Storage: 60% → 64% (4 cycles)
  - Cycle 20: Incremental snapshot creation via dirty-set (efficient O(delta) instead of O(total))
  - Cycle 21: Snapshot loader DB integration + 7 roundtrip tests (fix load_snapshot to insert into DB)
  - Cycle 22: AppendVec writer + Solana archive builder (tar construction, binary serialization, zstd)
  - Cycle 23: Solana-compatible snapshot archive creator with full roundtrip via SnapshotRestorer
- Total new tests: +31 (371 storage)
- See: `docs/plans/storage_snapshot_cycles_20_23.md`

Storage manifest cycles 25-27 (2026-02-18):
- Storage: 64% → 67% (3 cycles)
  - Cycle 25: Bank state manifest serializer (BincodeWriter promoted, serialize_bank_state, 8 roundtrip tests)
  - Cycle 26: Incremental Solana-compatible archives (create_incremental_solana_archive, dirty-set drain, 6 tests)
  - Cycle 27: AccountsDbFields serialization + full manifest (StorageEntry, AccountsDbLayout, serialize_full_manifest, 5 tests)
- Total new tests: +19 (396 storage)
- See: `docs/plans/storage_snapshot_cycles_25_27.md`

VM+Syscalls plan (2026-02-18, 6 cycles):
- Execution runtime (sBPF VM + syscalls): 55% → 80%
  - Cycle 1: 5 missing runtime/logging syscalls (sol_log_pubkey, get_epoch_rewards_sysvar, get_sysvar, get_processed_sibling_instruction, get_epoch_stake)
  - Cycle 2: ALT-BN128 group_op + compression + Poseidon hash syscalls (ark-bn254, light-poseidon)
  - Cycle 3: SBPF version-aware execution (V0-V3 conditional opcodes, stack gap handling, static syscall resolution)
  - Cycle 4: Memory model completion (zero-copy translate_slice/translate_type, alignment enforcement, unmapped region protection)
  - Cycle 5: TransactionProcessor ↔ BytecodeVm integration (proper BPF serialization via bpf_serialization.rs, InstructionExecutor trait for CPI, full pipeline tests)
  - Cycle 6: Feature-gated syscall registration (with_features() on RuntimeSyscallDispatch, 5 feature-gated syscalls, feature constants in paradencer-constants)
...
## Next Implementation Order (recommended, 2026-02-17)

Priority order based on critical path to working validator:

### ✅ Priority 1: VM + Syscalls (55% → 80%) — DONE (2026-02-18)
- ✅ Missing runtime/logging syscalls (sol_log_pubkey, get_epoch_rewards, get_sysvar, get_processed_sibling, get_epoch_stake)
- ✅ ALT-BN128 group ops, compression, Poseidon hash syscalls
- ✅ SBPF version-aware execution (V0-V3 conditional opcodes, stack gaps, static syscalls)
- ✅ Memory model completion (zero-copy translation, alignment checks, unmapped regions)
- ✅ TransactionProcessor ↔ BytecodeVm integration (proper BPF serialization, CPI executor)
- ✅ Feature-gated syscall registration (with_features() constructor, 5 gated syscalls)
- **Next**: Full CPI privilege checks, JIT compiler, remaining edge cases.

### ✅ Priority 2: Storage + Snapshot Restore (25% → 50%) — DONE
- ✅ Fork-aware transaction tree with ancestry-based account reads (funk-equivalent)
- ✅ Solana AppendVec binary format parser (136-byte header, streaming iterator)
- ✅ Solana snapshot archive reader (tar.zst decompression, entry classification)
- ✅ Owner-based secondary index with O(1) lamport/count stats and slot tracking
- ✅ FEC set completion tracking with duplicate detection and recovery signaling
- ✅ Typed shred insertion with auto slot completion and parent-child connectivity
- ✅ Snapshot restore pipeline (compressed archive → AppendVec → AccountDatabase)
- ✅ DurableStore trait + FileDurableStore sled backend (column families, batch writes, prefix/range scan)
- ✅ AccountDatabase persistence (disk fallback reads, publish persist, binary encode/decode, startup recovery)
- ✅ Blockstore persistence (persistent backend variant, root recovery on startup)
- ✅ StorageEngine facade (unified open/recover/compact/flush/disk_usage)
- ✅ Slot-keyed compaction across 8 column families
- ✅ Snapshot bank state manifest parser (custom BincodeReader, SnapshotBankState with full field coverage)
- **Next**: Status cache parsing, snapshot network download, full genesis bootstrap.

### Priority 3: Tile Pipeline (30% → 50%)
- Pack tile — full transaction scheduling with conflict detection
- Net tile — packet pipeline parity
- Metrics pipeline — production-grade telemetry
- Full replay semantics with deterministic bank execution
- **ROI**: Required for block production and validation.

### Priority 4: Types Completeness (55% → 75%)
- Full bincode-compatible type coverage for all protocol messages
- Transaction parsing (versioned transactions, address lookup tables)
- Gossip/repair message types
- **ROI**: Required for protocol compatibility.

### Priority 5: Network Protocol Behavior (30% → 50%)
- Gossip protocol fidelity (push/pull/prune, bloom filters, active set)
- Repair protocol fidelity (window/orphan/highest requests)
- Deeper QUIC session management
- **ROI**: Required for network participation.

### Priority 6: Consensus Deepening (70% → 85%)
- Tower BFT with optimistic confirmation pipeline
- Full fork choice with weighted voting
- Heavy fork detection and handling
- **ROI**: Required for consensus participation.

### Lower Priority:
- IPC (tango equivalent) — bounded channels vs shared memory decision
- Bundle/MEV support
- BLS signatures
- gRPC/Geyser API
- GUI dashboard (skip)
