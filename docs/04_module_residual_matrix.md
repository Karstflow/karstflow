...
- Storage + Snapshot restore pipeline: ~8% (vinyl persistent storage, snapshot network download)
...
| Storage / accounts (accdb, funk, vinyl, snapshots) | 8% | 50% | 4.0% |
| Network stack (QUIC, TLS, HTTP, gossip, repair) | 6% | 30% | 1.8% |
| App / config / IPC / observability | 6% | 25% | 1.5% |

Summary:
- Total migrated logic estimate: **~61.5%** (rounded tracking value: **~62%**).
...
2. **Storage** (25%) — no vinyl, snapshot restore minimal. Blocks mainnet bootstrap.
...
- **Next**: Vinyl-equivalent persistent storage, snapshot network download, full genesis bootstrap.
