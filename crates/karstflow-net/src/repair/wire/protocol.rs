...
impl WireRepairProtocol {
    ...
    pub fn sign(&mut self, secret_key: &[u8; 32]) {
        ...
        let signature = match karstflow_crypto::sign_message(secret_key, &signed_data) {
            Ok(sig) => sig,
            Err(_) => return,
        };
    ...
    pub fn verify(&self) -> bool {
        ...
        matches!(
            karstflow_crypto::verify_signature(&header.sender, &signed_data, &header.signature),
    ...
    pub fn is_timestamp_valid(&self, current_time_ms: u64) -> bool {
        ...
        let skew_ms = karstflow_constants::repair::REPAIR_TIMESTAMP_SKEW_SECS * 1000;
...
mod tests {
    ...
    fn discriminants_are_correct() {
        ...
        let (secret, pk) = karstflow_crypto::generate_keypair();
    ...
    fn window_index_fits_in_mtu() {
        ...
        let bytes = msg.encode().unwrap();
        assert!(
            bytes.len() <= karstflow_constants::gossip::GOSSIP_MTU,
            "WindowIndex {} bytes exceeds MTU {}",
            bytes.len(),
            karstflow_constants::gossip::GOSSIP_MTU,
        );
    }
