...
impl CrdsEntry {
    ...
    pub fn peer_score(&self, now_nanos: i64, fresh_threshold: i64) -> u64 {
        let base = karstflow_constants::gossip::PEER_SCORE_BASE_WEIGHT.saturating_add(self.stake);
        if self.is_stale(now_nanos, fresh_threshold) {
            base / karstflow_constants::gossip::OFFLINE_PEER_DOWNWEIGHT_FACTOR
        } else {
            base
        }
    }
...
mod tests {
    ...
    fn test_entry_expiration_staked() {
        ...
        let staked_ttl = karstflow_constants::gossip::STAKED_EXPIRE_DURATION_NANOS;
        let unstaked_ttl = karstflow_constants::gossip::UNSTAKED_EXPIRE_DURATION_NANOS;
    ...
    fn test_peer_score_fresh() {
        ...
        let threshold = karstflow_constants::gossip::FRESH_THRESHOLD_NANOS;
