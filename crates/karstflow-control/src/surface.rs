...
use karstflow_config::{validate_identity_keypair_file, NodeConfig};
...
pub fn validate_identity_keypair_from_env() -> Result<PathBuf> {
    let keypair_path = std::env::var("KARSTFLOW_IDENTITY_KEYPAIR_PATH")
        .ok()
...
mod tests {
    ...
    fn validate_identity_keypair_from_env_accepts_valid_keypair_file() {
        let keypair_path = unique_temp_file("karstflow-keypair", "json");
        ...
        unsafe {
            std::env::set_var("KARSTFLOW_IDENTITY_KEYPAIR_PATH", &keypair_path);
        }
        let result = validate_identity_keypair_from_env();
        unsafe {
            std::env::remove_var("KARSTFLOW_IDENTITY_KEYPAIR_PATH");
        }
