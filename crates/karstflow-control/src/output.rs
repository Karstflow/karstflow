use karstflow_config::ClusterMode;
use std::path::Path;
...
pub fn render_readiness_policy_line(
    scope: &str,
    readiness_policy: &karstflow_config::MainnetReadinessPolicy,
) -> String {
    format!(
...
pub fn render_pinned_affinity_line(
    ...
    assignment_source: karstflow_runtime::PinnedAssignmentSource,
    assigned_core_ids: &[usize],
) -> String {
    let assignment_source = match assignment_source {
        karstflow_runtime::PinnedAssignmentSource::Auto => "auto",
        karstflow_runtime::PinnedAssignmentSource::Explicit => "explicit",
    };
...
mod tests {
    ...
    fn output_renderers_produce_expected_strings() {
        ...
        assert_eq!(
            render_version_line("karstflow-node", "0.1.0"),
            "karstflow-node 0.1.0"
        );
        assert_eq!(
        ...
        let readiness_policy = karstflow_config::MainnetReadinessPolicy::default();
        assert_eq!(
        ...
        assert_eq!(
            render_pinned_affinity_line(
                "diagnostics",
                24,
                karstflow_runtime::PinnedAssignmentSource::Explicit,
                &[2, 4, 6, 8, 8]
            ),
