...
mod tests {
    ...
    fn parse_command_defaults_to_run() {
        let parsed = parse_command(args(&["karstflow-node"])).unwrap();
    ...
    fn parse_command_accepts_preflight() {
        let parsed = parse_command(args(&["karstflow-node", "preflight"])).unwrap();
    ...
    fn parse_command_accepts_diagnostics_aliases() {
        let diagnostics = parse_command(args(&["karstflow-node", "diagnostics"])).unwrap();
        let doctor = parse_command(args(&["karstflow-node", "doctor"])).unwrap();
    ...
    fn parse_command_accepts_config_path_for_diagnostics_command() {
        ...
        assert_eq!(
            ...
            std::path::PathBuf::from("/tmp/karstflow.toml")
        );
        assert_eq!(parsed.probe_ticks, 0);
        assert!(!parsed.mainnet_readiness);
    }
    ...
    fn parse_command_accepts_version_aliases() {
        let parsed = parse_command(args(&["karstflow-node", "-V"])).unwrap();
    ...
    fn parse_command_accepts_with_tick_for_preflight_and_diagnostics() {
        let preflight =
            parse_command(args(&["karstflow-node", "preflight", "--with-tick"])).unwrap();
        let diagnostics =
            parse_command(args(&["karstflow-node", "diagnostics", "--with-tick"])).unwrap();
    ...
    fn parse_command_accepts_config_and_keys() {
        let config_command = parse_command(args(&["karstflow-node", "config"])).unwrap();
        let keys_command = parse_command(args(&["karstflow-node", "keys"])).unwrap();
    ...
    fn parse_command_accepts_config_path_without_explicit_run_command() {
        let parsed =
            parse_command(args(&["karstflow-node", "--config", "/tmp/karstflow.toml"])).unwrap();
        assert_eq!(parsed.command, ControlCommand::Run);
        assert_eq!(
            parsed.config_path.unwrap(),
            std::path::PathBuf::from("/tmp/karstflow.toml")
    ...
    fn parse_command_rejects_unknown_command() {
        let result = parse_command(args(&["karstflow-node", "unknown"]));
        assert!(result.is_err());
    }

    #[test]
    fn parse_command_rejects_with_tick_for_run() {
        let result = parse_command(args(&["karstflow-node", "run", "--with-tick"]));
        assert!(result.is_err());
    }

    #[test]
    fn parse_command_rejects_probe_ticks_zero() {
        let result = parse_command(args(&["karstflow-node", "preflight", "--probe-ticks", "0"]));
        assert!(result.is_err());
    }

    #[test]
    fn parse_command_rejects_mainnet_readiness_for_non_diagnostics_command() {
        let result = parse_command(args(&["karstflow-node", "run", "--mainnet-readiness"]));
        assert!(result.is_err());
    }

    #[test]
    fn parse_command_accepts_genesis_cluster_with_node_count() {
        let parsed = parse_command(args(&["karstflow-node", "genesis", "cluster", "3"])).unwrap();
    ...
    fn parse_command_rejects_genesis_cluster_zero_nodes() {
        let result = parse_command(args(&["karstflow-node", "genesis", "cluster", "0"]));
        assert!(result.is_err());
    }

    #[test]
    fn parse_command_rejects_genesis_cluster_over_limit() {
        let result = parse_command(args(&["karstflow-node", "genesis", "cluster", "101"]));
        assert!(result.is_err());
    }

    #[test]
    fn parse_command_accepts_configure() {
        let parsed = parse_command(args(&["karstflow-node", "configure"])).unwrap();
        assert_eq!(parsed.command, ControlCommand::Configure);
    }

    #[test]
    fn parse_command_accepts_check_alias() {
        let parsed = parse_command(args(&["karstflow-node", "check"])).unwrap();
        assert_eq!(parsed.command, ControlCommand::Configure);
    }
    ...
    fn parse_command_accepts_monitor() {
        let parsed = parse_command(args(&["karstflow-node", "monitor"])).unwrap();
        assert_eq!(parsed.command, ControlCommand::Monitor);
    }

    #[test]
    fn parse_command_accepts_status_alias() {
        let parsed = parse_command(args(&["karstflow-node", "status"])).unwrap();
        assert_eq!(parsed.command, ControlCommand::Monitor);
    }

    #[test]
    fn parse_command_rejects_mainnet_readiness_for_monitor() {
        let result = parse_command(args(&["karstflow-node", "monitor", "--mainnet-readiness"]));
        assert!(result.is_err());
    }
