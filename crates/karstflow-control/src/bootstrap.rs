...
use karstflow_core::{ExecutionMode, LinkKind, PinnedCorePolicy, StageKind};
use karstflow_execution::{ExecutionBridge, SbpfBackend};
use karstflow_mesh::{bounded_link, DualReceiver, DualSender};
use karstflow_net::tile::{BridgeConfig, BridgeHandle};
use karstflow_net::{
...
use karstflow_topology::{materialize_services_with_blockstore, MaterializedTopology};
...
pub fn build_pipeline_service(
    ...
    execution_engine: Option<Box<dyn karstflow_stages::ExecutionEngine>>,
) -> PipelineBundle {
    ...
    if let Some(engine) = execution_engine {
        ...
        #[cfg(test)]
        {
            builder = builder.with_execution_engine(Box::new(
                karstflow_stages::MockExecutionEngine::new(200_000),
            ));
        }
        #[cfg(not(test))]
        {
...
pub struct ReplayBundle {
    ...
    pub block_input: DualSender<karstflow_stages::AssembledBlock>,
}
...
pub fn bootstrap_from_genesis_file(
    ...
    let genesis = karstflow_storage::genesis::parse_genesis(genesis_path).map_err(|e| {
    ...
    let result = karstflow_consensus::bootstrap_from_genesis(&genesis, leader_schedule);
...
pub fn development_faucet_pubkey() -> Pubkey {
    // SHA-256("karstflow-dev-faucet")[..32] — deterministic, reproducible.
    Pubkey::new([
...
pub fn build_replay_service(config: ReplayServiceConfig, initial_stake: u64) -> ReplayBundle {
    ...
    let (block_tx, block_rx) = bounded_link::<karstflow_stages::AssembledBlock>(channel_depth);
...
pub fn build_replay_service_with_block_input(
    ...
    block_input: DualReceiver<karstflow_stages::AssembledBlock>,
    initial_stake: u64,
...
pub fn resolve_validator_identity(node_config: &NodeConfig) -> Result<ValidatorIdentity> {
    if let Some(ref path) = node_config.identity_keypair_path {
        let identity = karstflow_config::load_identity_keypair(path).map_err(|e| {
            ControlPlaneError::Bootstrap {
        ...
        let derived_pubkey = karstflow_crypto::public_key_from_secret(identity.secret_key());
...
pub struct RepairBundle {
    ...
    pub repair_stats: Arc<karstflow_stages::AtomicRepairStats>,
}
...
struct RepairServiceAdapter {
    ...
    atomic_stats: Arc<karstflow_stages::AtomicRepairStats>,
...
impl RepairServiceAdapter {
    ...
    fn sync_peers_from_gossip(&mut self) {
        ...
        for contact in all_peers {
            ...
            let node_key = karstflow_types::Pubkey::new(peer_id);
...
pub fn build_repair_service(
    ...
    let (outbound_tx, outbound_rx) = crossbeam_channel::bounded::<OutboundRepair>(
        karstflow_constants::repair::REPAIR_OUTBOUND_CHANNEL_DEPTH,
    );

    let atomic_stats = Arc::new(karstflow_stages::AtomicRepairStats::default());
...
struct VoteBroadcastAdapter {
    tower: Arc<RwLock<karstflow_consensus::Tower>>,
    bank_forks: Arc<RwLock<karstflow_consensus::BankForks>>,
    cluster_info: Arc<ClusterInfo>,
    node_pubkey: [u8; 32],
...
impl Service for VoteBroadcastAdapter {
    ...
    fn tick(
        ...
        let vote_gossip = karstflow_net::CrdsValueData::Vote(karstflow_net::VoteGossip {
        ...
        let mut crds_value = karstflow_net::CrdsValue {
...
pub fn build_vote_transaction(
    ...
    node_pubkey: &[u8; 32],
    vote_account: &[u8; 32],
    votes: &[karstflow_consensus::TowerVote],
    root: Option<u64>,
    ...
    instr_data.extend_from_slice(
        &karstflow_constants::vote_program::INSTRUCTION_TOWER_SYNC.to_le_bytes(),
    );
    match root {
        Some(slot) => {
            instr_data.push(1);
    ...
    // Sign the message
    let signature = karstflow_crypto::sign_message(secret_key, &message).unwrap_or([0u8; 64]);
...
pub fn spawn_snapshot_thread(
    signal_bus: &Arc<Mutex<karstflow_stages::SignalBus>>,
    accounts: Option<Arc<AccountDatabase>>,
    bank_forks: Arc<RwLock<BankForks>>,
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            while let Ok(signal) = signal_rx.recv() {
                let karstflow_stages::ReplaySignal::RootAdvanced(root_info) = signal else {
                    continue;
                };
...
fn extract_bank_state_for_slot(
    bank_forks: &Arc<RwLock<BankForks>>,
    slot: u64,
) -> Option<karstflow_storage::SnapshotBankState> {
...
pub struct ShredPipelineBundle {
    ...
    pub shred_input: DualSender<karstflow_types::shred::Shred>,
    /// Receiver for assembled blocks (connect to ReplayService).
    pub block_receiver: DualReceiver<karstflow_stages::AssembledBlock>,
}
...
pub fn build_shred_pipeline(
    ...
    let (shred_tx, shred_rx) = bounded_link::<karstflow_types::shred::Shred>(shred_channel_depth);
    let (block_tx, block_rx) =
        bounded_link::<karstflow_stages::AssembledBlock>(block_channel_depth);
...
pub struct ShredPipelineLinkBundle {
    ...
    pub shred_link: Box<karstflow_mesh::tile_link::TileLink>,
    /// Receiver for assembled blocks (connect to ReplayService).
    pub block_receiver: DualReceiver<karstflow_stages::AssembledBlock>,
}
...
pub fn build_shred_pipeline_with_link(
    ...
    let (_shred_tx, shred_rx) = bounded_link::<karstflow_types::shred::Shred>(shred_channel_depth);
    let (block_tx, block_rx) =
        bounded_link::<karstflow_stages::AssembledBlock>(block_channel_depth);
    ...
    let shred_link = Box::new(karstflow_stages::shred_link::new_shred_link());
...
pub fn maybe_spawn_quic_bridge(
    ...
    let handle = karstflow_net::tile::spawn_bridge(bridge_config).map_err(|e| {
        ControlPlaneError::Bootstrap {
        ...
        .spawn(move || {
            while let Ok(quic_tx) = tx_rx.recv() {
                let raw_tx = RawTransaction {
                    ...
                    source: karstflow_stages::TransactionSource::Quic,
                };
...
pub fn maybe_start_rpc_http_server_with_consensus(
    ...
    let bind_addr = node_config.rpc_bind.ok_or(ControlPlaneError::Config(
        karstflow_config::ConfigError::RpcEnabledRequiresBindAddr,
    ))?;

    let (runtime_snapshot_provider, bank_access_provider, tx_submitter) =
        if let Some(ref forks) = bank_forks {
            let snap: Option<Arc<dyn karstflow_rpc::RuntimeSnapshotProvider>> =
...
impl karstflow_rpc::RuntimeSnapshotProvider for ConsensusSnapshotProvider {
    fn latest_snapshot(&self) -> Option<karstflow_rpc::RpcRuntimeSnapshot> {
        let forks = self.bank_forks.read().ok()?;
        let bank = forks.working_bank();
        Some(karstflow_rpc::RpcRuntimeSnapshot {
            slot: bank.slot(),
...
impl ConsensusBankAccessProvider {
    ...
    fn bank_for_commitment(&self, commitment: karstflow_rpc::RpcCommitment) -> Option<Arc<Bank>> {
        let forks = self.bank_forks.read().ok()?;
    ...
    fn bank_for_commitment_inner(
        ...
        match commitment {
            karstflow_rpc::RpcCommitment::Finalized => {
                forks.root_bank().unwrap_or_else(|| forks.working_bank())
            }
            karstflow_rpc::RpcCommitment::Confirmed => {
            ...
            karstflow_rpc::RpcCommitment::Processed => forks.working_bank(),
        }
    }
}
...
impl BankAccessProvider for ConsensusBankAccessProvider {
    ...
    fn get_slot(&self, commitment: karstflow_rpc::RpcCommitment) -> u64 {
        self.bank_for_commitment(commitment)
            .map(|bank| bank.slot())
            .unwrap_or(0)
    }

    fn get_block_height(&self, commitment: karstflow_rpc::RpcCommitment) -> u64 {
    ...
    fn get_lamports_per_signature(&self, commitment: karstflow_rpc::RpcCommitment) -> u64 {
        self.bank_for_commitment(commitment)
    ...
    fn get_transaction_count(&self, commitment: karstflow_rpc::RpcCommitment) -> u64 {
        self.bank_for_commitment(commitment)
            .map(|bank| bank.transaction_count())
            .unwrap_or(0)
    }

    fn get_capitalization(&self, commitment: karstflow_rpc::RpcCommitment) -> u64 {
        self.bank_for_commitment(commitment)
    ...
    fn simulate_transaction(
        ...
        commitment: karstflow_rpc::RpcCommitment,
    ) -> karstflow_rpc::TransactionSimulationResponse {
        let bank = match self.bank_for_commitment(commitment) {
            Some(bank) => bank,
            None => {
                return karstflow_rpc::TransactionSimulationResponse {
    ...
    fn get_cluster_nodes(&self) -> Vec<karstflow_rpc::RpcClusterNode> {
        ...
        table
            ...
            .filter_map(|entry| {
                ...
                let gossip = contact
                    .sockets
                    .get(karstflow_constants::gossip::SOCKET_GOSSIP)
                    .copied()
                    .flatten()
                    .map(|a| a.to_string());
                let tpu = contact
                    .sockets
                    .get(karstflow_constants::gossip::SOCKET_TPU)
                    .copied()
                    .flatten()
                    .map(|a| a.to_string());
                let rpc = contact
                    .sockets
                    .get(karstflow_constants::gossip::SOCKET_RPC)
                    .copied()
                    .flatten()
                    .map(|a| a.to_string());
                let version = {
                ...
                Some(karstflow_rpc::RpcClusterNode {
                    pubkey,
                    gossip,
                    tpu,
                    rpc,
    ...
    fn get_largest_accounts(
        ...
        limit: usize,
        commitment: karstflow_rpc::RpcCommitment,
    ) -> Vec<(karstflow_types::Pubkey, u64)> {
        let bank = match self.bank_for_commitment(commitment) {
            Some(bank) => bank,
            None => return Vec::new(),
        };
        let db = bank.accounts();
        let mut all_accounts: Vec<(karstflow_types::Pubkey, u64)> = db
    ...
    fn get_non_circulating_supply(&self, commitment: karstflow_rpc::RpcCommitment) -> u64 {
        let bank = match self.bank_for_commitment(commitment) {
            Some(bank) => bank,
            None => return 0,
        };
        ...
        let vote_total: u64 = db
            .get_accounts_by_owner(&karstflow_ids::VOTE_PROGRAM_ID)
            .iter()
            .map(|(_, acct)| acct.meta.lamports)
            .sum();
        let stake_total: u64 = db
            .get_accounts_by_owner(&karstflow_ids::STAKE_PROGRAM_ID)
            .iter()
            .map(|(_, acct)| acct.meta.lamports)
            .sum();
    ...
    fn get_block_commitment(&self, slot: u64) -> Option<karstflow_rpc::RpcBlockCommitment> {
        ...
        Some(karstflow_rpc::RpcBlockCommitment {
            commitment,
    ...
    fn get_inflation_rate(&self, epoch: u64) -> Option<(f64, f64, f64)> {
        ...
        let year = (epoch as f64 * slots_per_epoch)
            / karstflow_constants::economics::DEFAULT_SLOTS_PER_YEAR;
    ...
    fn get_signature_statuses(
        &self,
        signatures: &[[u8; 64]],
    ) -> Vec<Option<karstflow_rpc::RpcSignatureStatus>> {
        let forks = match self.bank_forks.read() {
            Ok(f) => f,
        ...
        bank.signature_status_cache()
            ...
            .into_iter()
            .map(|opt| {
                opt.map(|s| karstflow_rpc::RpcSignatureStatus {
                    slot: s.slot,
    ...
    fn get_transaction(&self, signature: &[u8; 64]) -> Option<karstflow_rpc::RpcTransactionData> {
        ...
        for entry in &entries {
            for tx_bytes in &entry.transactions {
                let sigs = karstflow_storage::extract_signatures(tx_bytes).unwrap_or_default();
                if sigs
                    .first()
                    ...
                    .unwrap_or(false)
                {
                    ...
                    return Some(karstflow_rpc::RpcTransactionData {
                        slot: status.slot,
                        block_time,
        ...
        Some(karstflow_rpc::RpcTransactionData {
            slot: status.slot,
            block_time,
            succeeded: status.succeeded,
            error: status.error,
    ...
    fn get_signatures_for_address(
        &self,
        address: &karstflow_types::Pubkey,
        limit: usize,
        before: Option<&[u8; 64]>,
        until: Option<&[u8; 64]>,
        _commitment: karstflow_rpc::RpcCommitment,
    ) -> Vec<karstflow_rpc::RpcAddressSignatureEntry> {
        let forks = match self.bank_forks.read() {
            Ok(f) => f,
        ...
        entries
            ...
            .map(|e| {
                ...
                karstflow_rpc::RpcAddressSignatureEntry {
    ...
    fn get_recent_prioritization_fees(
        &self,
        commitment: karstflow_rpc::RpcCommitment,
    ) -> Vec<karstflow_rpc::RpcPrioritizationFee> {
        let forks = match self.bank_forks.read() {
            Ok(f) => f,
            Err(_) => return Vec::new(),
        };
        ...
        for slot in (start..=current_slot).rev() {
            if let Some(slot_bank) = forks.get(slot) {
                fees.push(karstflow_rpc::RpcPrioritizationFee {
                    slot,
    ...
    fn get_recent_performance_samples(
        ...
        limit: usize,
        commitment: karstflow_rpc::RpcCommitment,
    ) -> Vec<karstflow_rpc::RpcPerformanceSample> {
        let forks = match self.bank_forks.read() {
            Ok(f) => f,
            Err(_) => return Vec::new(),
        };
        ...
        for slot in (start..=current_slot).rev() {
            if let Some(slot_bank) = forks.get(slot) {
                samples.push(karstflow_rpc::RpcPerformanceSample {
                    slot,
    ...
    fn get_block_data(&self, slot: u64) -> Option<karstflow_rpc::RpcBlockData> {
        use karstflow_storage::extract_signatures;
        ...
        for entry in &entries {
            for tx_bytes in &entry.transactions {
                let sigs = extract_signatures(tx_bytes)
                    ...
                    .collect();
                transactions.push(karstflow_rpc::RpcBlockTransaction {
                    signatures: sigs,
                    raw_bytes: tx_bytes.clone(),
                });
            }
        ...
        Some(karstflow_rpc::RpcBlockData {
            slot,
...
impl karstflow_stages::LeaderLookup for ConsensusLeaderLookup {
...
pub struct LocalTransactionSubmitter {
    sender: karstflow_mesh::OutPort<karstflow_stages::RawTransaction>,
}
...
pub fn build_local_transaction_submitter() -> (
    ...
    DualReceiver<karstflow_stages::RawTransaction>,
) {
    let (tx, rx) = bounded_link::<karstflow_stages::RawTransaction>(256);
...
impl TransactionSubmitter for LocalTransactionSubmitter {
    fn submit_transaction(&self, tx_bytes: &[u8]) -> std::result::Result<[u8; 64], String> {
        ...
        let raw_tx = karstflow_stages::RawTransaction {
