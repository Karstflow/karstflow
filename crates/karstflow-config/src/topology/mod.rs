...
pub fn build_topology_spec(profile: Option<&NodeProfileToml>) -> Result<TopologySpec> {
    ...
    if let Some(value) = parse_optional_usize_env("KARSTFLOW_PACKET_LINK_CAPACITY")? {
        packet_link_capacity = value;
    }
    if let Some(value) = parse_optional_usize_env("KARSTFLOW_SHRED_LINK_CAPACITY")? {
        shred_link_capacity = value;
    }
    if let Some(value) = parse_optional_usize_env("KARSTFLOW_TRANSACTION_LINK_CAPACITY")? {
        transaction_link_capacity = value;
    }
    if let Some(value) = parse_optional_usize_env("KARSTFLOW_TRANSACTION_SANITIZER_WORKERS")? {
        transaction_sanitizer_workers = value;
    }
    if let Some(value) = parse_optional_usize_env("KARSTFLOW_SHRED_SANITIZER_WORKERS")? {
