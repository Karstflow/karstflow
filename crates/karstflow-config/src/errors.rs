...
pub enum ConfigError {
    ...
    #[error("KARSTFLOW_METRICS_TARGET=file requires KARSTFLOW_METRICS_FILE_PATH")]
    MetricsEnvFileTargetRequiresPath,
    #[error("KARSTFLOW_METRICS_TARGET=udp requires KARSTFLOW_METRICS_UDP_ADDR")]
    MetricsEnvUdpTargetRequiresAddress,
    #[error("rpc.enabled=true requires rpc.bind (or KARSTFLOW_RPC_BIND)")]
    ...
    #[error("live mode requires KARSTFLOW_IDENTITY_KEYPAIR_PATH")]
    ...
    #[error("live mode requires KARSTFLOW_EXPECTED_GENESIS_HASH")]
    LiveModeRequiresExpectedGenesisHash,
    #[error("live mode requires KARSTFLOW_EXPECTED_SHRED_VERSION")]
    LiveModeRequiresExpectedShredVersion,
    #[error("live mode requires KARSTFLOW_EXPECTED_SHRED_VERSION to be greater than zero")]
    LiveModeRequiresPositiveShredVersion,
    #[error(
        "live mode requires KARSTFLOW_EXPECTED_GENESIS_HASH to be a 64-char lowercase hex string"
    )]
    LiveModeInvalidExpectedGenesisHash,
    #[error("live mode requires KARSTFLOW_LIVE_ENTRYPOINTS")]
