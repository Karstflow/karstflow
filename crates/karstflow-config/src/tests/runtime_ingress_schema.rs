...
fn identity_keypair_validator_accepts_64_byte_json_array() {
    let keypair_path = unique_temp_file("karstflow-keypair-valid", "json");
...
fn identity_keypair_validator_rejects_short_json_array() {
    let keypair_path = unique_temp_file("karstflow-keypair-short", "json");
...
fn identity_keypair_validator_rejects_invalid_json_shape() {
    let keypair_path = unique_temp_file("karstflow-keypair-invalid", "json");
...
fn storage_preflight_rejects_missing_catalog_parent_directory() {
    ...
    let catalog_path = std::env::temp_dir()
        .join("karstflow-storage-missing-parent")
        .join("catalog.json");
...
fn storage_preflight_rejects_strict_restore_without_existing_catalog_file() {
    let policy = StorageRuntimePolicy {
        ...
        startup_strict_restore_policy: karstflow_stages::StorageStartupStrictRestorePolicy {
            restore_latest_requires_snapshot: true,
            restore_specific_requires_snapshot: false,
        },
        snapshot_catalog_path: Some(unique_temp_file("karstflow-missing-catalog", "json")),
...
fn storage_preflight_accepts_strict_restore_when_catalog_file_exists() {
    let catalog_path = unique_temp_file("karstflow-existing-catalog", "json");
    fs::write(
        &catalog_path,
...
fn storage_preflight_rejects_existing_catalog_with_invalid_checksum() {
    let catalog_path = unique_temp_file("karstflow-invalid-checksum-catalog", "json");
    fs::write(
        &catalog_path,
...
fn storage_preflight_rejects_existing_catalog_with_header_last_fragment_mismatch() {
    let catalog_path = unique_temp_file("karstflow-invalid-header-last-fragment-catalog", "json");
    fs::write(
        &catalog_path,
...
fn storage_preflight_rejects_existing_catalog_with_header_written_count_mismatch() {
    let catalog_path = unique_temp_file("karstflow-invalid-header-written-count-catalog", "json");
    fs::write(
        &catalog_path,
...
fn metrics_preflight_rejects_missing_file_parent_directory() {
    let target = MetricsOutputTarget::File(
        std::env::temp_dir()
            .join("karstflow-metrics-missing-parent")
            .join("metrics.jsonl"),
    );
...
fn metrics_preflight_accepts_well_formed_targets() {
    let file_target = MetricsOutputTarget::File(unique_temp_file("karstflow-metrics", "jsonl"));
...
catalog_path = "/tmp/karstflow-snapshot-catalog.json"
...
"#,
    ...
    assert_eq!(
        ...
        Some("/tmp/karstflow-snapshot-catalog.json".to_string())
    );
...
fn node_config_from_file_loads_profile_and_builds_storage_policy() {
    let config_path = unique_temp_file("karstflow-node-config", "toml");
    fs::write(
        &config_path,
        r#"
[storage]
