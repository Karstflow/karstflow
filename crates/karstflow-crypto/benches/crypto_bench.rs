...
//!
//! Run with: cargo bench --package karstflow-crypto
...
fn bench_transaction_signature_hashing(c: &mut Criterion) {
    ...
    c.bench_function("hash_transaction_signature", |b| {
        b.iter(|| {
            let hash = karstflow_crypto::blake3::hash_transaction_signature(black_box(&signature));
            black_box(hash);
        })
    });
}
...
fn bench_shred_fingerprint_hashing(c: &mut Criterion) {
    ...
    c.bench_function("hash_shred_fingerprint", |b| {
        b.iter(|| {
            let hash = karstflow_crypto::blake3::hash_shred_fingerprint(
                black_box(slot),
                black_box(index),
                black_box(&data),
            );
    ...
    c.bench_function("dedup_hash_1000_signatures", |b| {
        b.iter(|| {
            for signature in &signatures {
                let hash =
                    karstflow_crypto::blake3::hash_transaction_signature(black_box(signature));
                black_box(hash);
            }
        })
    });
