...
fn metrics_reporter_writes_json_metrics_to_file_target() {
    ...
    let metrics_path = unique_temp_file("paradencer-metrics", "jsonl");
...
fn metrics_reporter_returns_runtime_error_when_file_target_cannot_be_opened() {
    ...
    let (shred_outbound, _shred_inbound) = bounded_link::<InboundPacket>(8);
    let (transaction_outbound, _transaction_inbound) = bounded_link::<SanitizedTransaction>(8);
    let missing_parent = unique_temp_file("paradencer-missing-parent", "dir");
...
fn metrics_reporter_exports_block_assembly_leader_and_fork_counters() {
    ...
    let (shred_outbound, _shred_inbound) = bounded_link::<InboundPacket>(8);
    let (transaction_outbound, _transaction_inbound) = bounded_link::<SanitizedTransaction>(8);
    let metrics_path = unique_temp_file("paradencer-metrics-prom", "txt");
    ...
    let content = fs::read_to_string(&metrics_path).unwrap();
    fs::remove_file(&metrics_path).unwrap();
    assert!(content.contains("paradencer_block_assembly_leader_gate_permit_total 1"));
    assert!(content.contains("paradencer_block_assembly_leader_gate_hold_total 1"));
    assert!(content.contains("paradencer_block_assembly_fork_choice_keep_total 1"));
    assert!(content.contains("paradencer_block_assembly_fork_choice_reorg_total 1"));
    assert!(
        content.contains("paradencer_block_assembly_execution_health_cooldown_entries_total 1",)
    );
    assert!(content
        .contains("paradencer_block_assembly_execution_health_cooldown_skipped_ticks_total 1",));
    assert!(content.contains("paradencer_block_assembly_replay_safety_hold_entries_total 1"));
    assert!(content.contains("paradencer_block_assembly_replay_safety_hold_skipped_ticks_total 1"));
    assert!(content.contains("paradencer_block_assembly_fork_choice_quarantine_entries_total 1"));
    assert!(
        content.contains("paradencer_block_assembly_fork_choice_quarantine_skipped_ticks_total 1")
    );
    assert!(content.contains("paradencer_block_assembly_slot_transition_committed_total 1"));
    assert!(content.contains("paradencer_block_assembly_slot_transition_dropped_total 1"));
    assert!(content.contains("paradencer_block_assembly_slot_transition_reorg_pending_total 1"));
    assert!(content.contains("paradencer_block_assembly_execution_error_total 3"));
    assert!(content.contains("paradencer_block_assembly_execution_error_adapter_apply_total 1"));
    assert!(content
        .contains("paradencer_block_assembly_execution_error_adapter_state_conflict_total 1"));
    assert!(content
        .contains("paradencer_block_assembly_execution_error_adapter_contract_violation_total 1"));
    assert!(
        content.contains("paradencer_block_assembly_execution_error_fail_open_continue_total 1")
    );
    assert!(content.contains("paradencer_block_assembly_execution_error_fail_fast_halt_total 1"));
    assert!(content.contains(
        "paradencer_block_assembly_execution_error_fail_open_circuit_breaker_halt_total 1"
    ));
    assert!(content.contains("paradencer_block_assembly_execution_error_consecutive_current 2"));
    assert!(content.contains("paradencer_block_assembly_retry_scheduled_replay_conflict_total 1"));
    assert!(
        content.contains("paradencer_block_assembly_retry_scheduled_transient_pressure_total 1")
    );
    assert!(
        content.contains("paradencer_block_assembly_retry_scheduled_resource_exhaustion_total 1")
    );
    assert!(content.contains("paradencer_block_assembly_retry_scheduled_fallback_total 1"));
    assert!(content.contains("paradencer_block_assembly_dropped_replay_conflict_total 1"));
    assert!(content.contains("paradencer_block_assembly_dropped_deterministic_total 1"));
    assert!(content.contains("paradencer_block_assembly_dropped_transient_pressure_total 1"));
    assert!(content.contains("paradencer_block_assembly_dropped_resource_exhaustion_total 1"));
    assert!(content.contains("paradencer_block_assembly_dropped_fallback_total 1"));
    assert!(content.contains("paradencer_block_assembly_dropped_reorg_retry_exhausted_total 1"));
    assert!(content.contains("paradencer_block_assembly_replay_controller_holds_total 1"));
    assert!(content
        .contains("paradencer_block_assembly_replay_controller_confirmed_candidates_total 1"));
    assert!(
        content.contains("paradencer_block_assembly_replay_controller_candidate_switches_total 1")
    );
    assert!(content
        .contains("paradencer_block_assembly_replay_controller_stale_candidates_pruned_total 2"));
    assert!(
        content.contains("paradencer_block_assembly_replay_controller_switch_suppressed_total 1")
    );
    assert!(content.contains(
        "paradencer_block_assembly_replay_controller_active_candidate_failed_ratio_bps 1337"
    ));
    assert!(content.contains("paradencer_block_assembly_replay_controller_tracked_candidates 4"));
    assert!(content.contains("paradencer_block_assembly_replay_window_checkpoint_depth 3"));
    assert!(content.contains("paradencer_block_assembly_replay_window_rewinds_total 1"));
    assert!(content
        .contains("paradencer_block_assembly_replay_window_catalog_snapshots_pruned_total 2"));
    assert!(content.contains("paradencer_block_assembly_execution_state_rewind_failures_total 1"));
    assert!(content.contains("paradencer_ingress_dropped_cost_budget_source"));
    assert!(content.contains("paradencer_ingress_dropped_downstream_backpressure"));
    assert!(content.contains("paradencer_shred_accepted"));
}
