...
impl WirePing {
    ...
    fn sign(&mut self, secret_key: &[u8; 32]) {
        self.signature = paradencer_crypto::sign_message(secret_key, &self.token)
    ...
    pub fn verify(&self) -> bool {
        matches!(
            paradencer_crypto::verify_signature(&self.from, &self.token, &self.signature),
...
impl WirePong {
    ...
    /// Sign the pong hash.
    fn sign(&mut self, secret_key: &[u8; 32]) {
        self.signature = paradencer_crypto::sign_message(secret_key, &self.hash)
            .expect("Ed25519 signing should never fail with a valid key");
    }
    ...
    pub fn verify(&self) -> bool {
        matches!(
            paradencer_crypto::verify_signature(&self.from, &self.hash, &self.signature),
            Ok(paradencer_crypto::VerificationResult::Success)
        )
    }
