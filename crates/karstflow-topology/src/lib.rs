...
mod tests {
    ...
    use karstflow_core::IpcMode;
    use karstflow_net::IngressPolicy;
    use karstflow_stages::{MetricsOutputFormat, MetricsOutputTarget, StorageRuntimePolicy};
    use std::fs;
    ...
    fn loader_parses_external_topology_file() {
        let suffix = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let temp_path = std::env::temp_dir().join(format!("karstflow-topology-{suffix}.toml"));
...
"#;
    ...
    fn loader_rejects_external_topology_with_unwired_transaction_worker() {
        let suffix = SystemTime::now()
            ...
            .as_nanos();
        let temp_path =
            std::env::temp_dir().join(format!("karstflow-topology-invalid-{suffix}.toml"));
...
"#;
    ...
    fn loader_rejects_transaction_stream_not_pointing_to_block_builder() {
        let suffix = SystemTime::now()
            ...
            .as_nanos();
        let temp_path =
            std::env::temp_dir().join(format!("karstflow-topology-invalid-dst-{suffix}.toml"));
