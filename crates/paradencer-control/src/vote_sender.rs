...
use paradencer_config::ValidatorIdentity;
use paradencer_consensus::{BankForks, Tower};
use paradencer_constants::gossip::SOCKET_TPU_VOTE;
use paradencer_constants::vote_sender::{POLL_INTERVAL_MS, TARGET_LEADER_COUNT};
use paradencer_net::ClusterInfo;
...
pub struct VoteSenderBundle {
    pub service: Box<dyn paradencer_runtime::Service>,
}
...
impl paradencer_runtime::Service for VoteSenderService {
    ...
    fn tick(
        ...
        let tower = self.tower.read().map_err(|_| {
            paradencer_runtime::RuntimeError::service_failure("vote-sender", "tower lock poisoned")
        })?;
...
mod tests {
    use super::*;
    use paradencer_consensus::TowerVote;
    use paradencer_runtime::Service;
    use std::net::SocketAddr;

    fn test_bank_forks() -> Arc<RwLock<BankForks>> {
        let db = Arc::new(paradencer_storage::AccountDatabase::new());
        let epoch_schedule = Arc::new(paradencer_consensus::EpochSchedule::default());
        let validator = Pubkey::new([0u8; 32]);
        let ls =
            Arc::new(paradencer_consensus::LeaderSchedule::new(0, &[(validator, 1000)]).unwrap());
        let bank = paradencer_consensus::Bank::new_genesis(db, epoch_schedule, ls);
        Arc::new(RwLock::new(BankForks::new(bank)))
    }

    fn test_cluster_info(pubkey: [u8; 32]) -> Arc<ClusterInfo> {
        let node_id = paradencer_net::NodeId::new(pubkey);
        let addr: SocketAddr = "127.0.0.1:8000".parse().unwrap();
        let contact_info = paradencer_net::ContactInfo::new(node_id, addr, addr, addr, addr, 0);
    ...
    fn vote_sender_skips_when_no_new_vote() {
        ...
        let ctx =
            paradencer_runtime::ServiceContext::new(paradencer_runtime::ShutdownSwitch::new());
