...
fn run_genesis_cluster(params: GenesisClusterParams) -> Result<()> {
    ...
    println!("=== Paradencer Local Cluster Genesis ===\n");
    println!(
    ...
    // Write genesis.bin.
    let genesis_data = paradencer_storage::genesis::serialize_genesis(&genesis).map_err(|e| {
        crate::errors::ControlPlaneError::Bootstrap {
            message: format!("genesis serialization failed: {e}"),
        }
    })?;
    ...
    let mut start_lines: Vec<String> = vec![
        ...
        "# Auto-generated by `paradencer-node genesis cluster`".to_string(),
        "set -e".to_string(),
    ...
    for (i, (secret_key, pubkey_bytes)) in keypairs.iter().enumerate() {
        ...
        // All nodes point to node 0's gossip address as their entrypoint.
        let entrypoints = format!("addresses = [\"{bootstrap_gossip}\"]");

        // Write per-node TOML config.
        let config_content = format!(
            "# Node {i} configuration — generated by genesis cluster\n\
             ...
             \n\
             [storage]\n\
             startup_policy = \"restore_genesis\"\n\
             genesis_path = \"${{CLUSTER_DIR}}/genesis.bin\"\n\
             data_dir = \"${{CLUSTER_DIR}}/node{i}/data\"\n\
             \n\
             [rpc]\n\
             enabled = true\n\
             ...
             [ingress]\n\
             mode = \"udp\"\n\
             udp_bind = \"0.0.0.0:{tpu_port}\"\n\
             \n\
             [identity]\n\
             keypair_path = \"${{CLUSTER_DIR}}/node{i}/identity.json\"\n\
             \n\
             [logging]\n\
             stderr_level = \"info\"\n"
        );
        ...
        start_lines.push(format!(
            "PARADENCER_NODE_CONFIG_PATH=\"$CLUSTER_DIR/node{i}/config.toml\" \\\n  \
             cargo run -p paradencer-node &"
        ));
    ...
    for i in 0..node_count {
        println!(
            "  PARADENCER_NODE_CONFIG_PATH={} cargo run -p paradencer-node",
