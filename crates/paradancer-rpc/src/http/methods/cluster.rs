...
fn build_signatures_for_address_response(
    ...
    let values: Vec<serde_json::Value> = (0_u64..entry_count as u64)
        .filter_map(|index| {
...
fn build_block_production_response(
    ...
    let mut by_identity = serde_json::Map::new();
    by_identity.insert(identity.to_string(), types::to_value(&[produced_count, produced_count]));
    let value = BlockProductionValue {
        by_identity: serde_json::Value::Object(by_identity),
        range: BlockProductionRange {
            first_slot,
            last_slot: effective_last_slot,
        },
