...
impl SignService {
    ...
    pub fn add_authorized_voter(&mut self, voter_key: SigningKey) -> Option<usize> {
        if self.authorized_voters.len() >= karstflow_constants::signing::MAX_AUTHORIZED_VOTERS {
            return None;
        }
...
mod tests {
    ...
    fn add_authorized_voters_up_to_limit() {
        let mut service = make_service();
        for i in 0..karstflow_constants::signing::MAX_AUTHORIZED_VOTERS {
        ...
        assert_eq!(
            ...
            karstflow_constants::signing::MAX_AUTHORIZED_VOTERS
        );
    }
