...
use karstflow_mesh::fragment::ctl_pack;
use karstflow_mesh::tile_link::{TileLink, TileLinkConfig};
...
pub fn ctl_to_shred_source(ctl: u16) -> ShredSource {
    let origin = karstflow_mesh::fragment::ctl_origin(ctl);
    match origin {
...
mod tests {
    use super::*;
    use karstflow_mesh::tile_link::ReceiveResult;
    ...
    fn ctl_has_som_and_eom() {
        ...
        assert!(karstflow_mesh::fragment::ctl_som(ctl));
        assert!(karstflow_mesh::fragment::ctl_eom(ctl));
        assert!(!karstflow_mesh::fragment::ctl_err(ctl));
    }
