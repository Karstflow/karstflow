...
use karstflow_mesh::DualReceiveError;
use karstflow_net::{DecodeOutcome, DedupDecision, IngressSource};
            ...
            Ok(Some(packet)) => {
                ...
                if !self.source_rate_limiter.try_accept(
                    ...
                    self.ingress_filter_stats.increment_drop_reason(
                        karstflow_net::DropReason::SourceRateLimited,
                        transaction.source,
                    );
                    return Ok(());
                }
                ...
                if !self.source_cost_budget_limiter.try_consume(
                    ...
                    self.ingress_filter_stats.increment_drop_reason(
                        karstflow_net::DropReason::SourceCostBudgetExceeded,
                        transaction.source,
                    );
                    return Ok(());
                }
