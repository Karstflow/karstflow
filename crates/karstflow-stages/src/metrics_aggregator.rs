...
use karstflow_storage::blockstore::{BlockstoreStats, BlockstoreStatsSnapshot};
...
/// on demand without depending on the `karstflow-net` crate directly.
...
impl AggregatedSnapshot {
    ...
    pub fn to_prometheus_lines(&self) -> Vec<String> {
        ...
        if let Some(ref v) = self.verify {
            lines.push(format!(
                "karstflow_verify_transactions_received {}",
                v.transactions_received
            ));
            lines.push(format!(
                "karstflow_verify_transactions_verified {}",
                v.transactions_verified
            ));
            lines.push(format!(
                "karstflow_verify_transactions_failed {}",
                v.transactions_failed
            ));
            lines.push(format!(
                "karstflow_verify_transactions_malformed {}",
                v.transactions_malformed
            ));
            lines.push(format!(
                "karstflow_verify_transactions_filtered {}",
                v.transactions_filtered
            ));
            lines.push(format!(
                "karstflow_verify_batches_processed {}",
                v.batches_processed
            ));
            lines.push(format!(
                "karstflow_verify_gossip_votes_received {}",
                v.gossip_votes_received
            ));
        }

        if let Some(ref d) = self.dedup {
            lines.push(format!("karstflow_dedup_total_checked {}", d.total_checked));
            lines.push(format!(
                "karstflow_dedup_duplicates_found {}",
                d.duplicates_found
            ));
            lines.push(format!("karstflow_dedup_unique_passed {}", d.unique_passed));
        }

        if let Some(ref r) = self.resolv {
            lines.push(format!(
                "karstflow_resolv_transactions_received {}",
                r.transactions_received
            ));
            lines.push(format!(
                "karstflow_resolv_transactions_resolved {}",
                r.transactions_resolved
            ));
            lines.push(format!(
                "karstflow_resolv_transactions_expired {}",
                r.transactions_expired
            ));
            lines.push(format!(
                "karstflow_resolv_transactions_stashed {}",
                r.transactions_stashed
            ));
            lines.push(format!(
                "karstflow_resolv_transactions_dropped {}",
                r.transactions_dropped
            ));
            lines.push(format!(
                "karstflow_resolv_blockhashes_registered {}",
        ...
        if let Some(ref p) = self.pack {
            lines.push(format!(
                "karstflow_pack_transactions_scheduled {}",
                p.transactions_scheduled
            ));
            lines.push(format!(
                "karstflow_pack_transactions_conflicted {}",
                p.transactions_conflicted
            ));
            lines.push(format!(
                "karstflow_pack_transactions_expired {}",
                p.transactions_expired
            ));
            lines.push(format!(
                "karstflow_pack_microblocks_produced {}",
                p.microblocks_produced
            ));
            lines.push(format!(
                "karstflow_pack_blocks_completed {}",
                p.blocks_completed
            ));
            lines.push(format!(
                "karstflow_pack_block_cost_units_used {}",
                p.block_cost_units_used
            ));
            lines.push(format!("karstflow_pack_rebated_cus {}", p.rebated_cus));
            lines.push(format!(
                "karstflow_pack_microblocks_paced {}",
                p.microblocks_paced
            ));
        }
        ...
        if let Some(ref e) = self.exec {
            lines.push(format!(
                "karstflow_exec_microblocks_executed {}",
                e.microblocks_executed
            ));
            lines.push(format!(
                "karstflow_exec_transactions_executed {}",
                e.transactions_executed
            ));
            lines.push(format!(
                "karstflow_exec_transactions_succeeded {}",
                e.transactions_succeeded
            ));
            lines.push(format!(
                "karstflow_exec_transactions_failed {}",
                e.transactions_failed
            ));
            lines.push(format!(
                "karstflow_exec_compute_units_consumed {}",
                e.compute_units_consumed
            ));
            lines.push(format!(
                "karstflow_exec_compute_units_rebated {}",
                e.compute_units_rebated
            ));
            lines.push(format!(
                "karstflow_exec_fees_collected {}",
                e.fees_collected
            ));
            // Landing classification.
            lines.push(format!(
                "karstflow_exec_transactions_landed {}",
                e.transactions_landed
            ));
            lines.push(format!(
                "karstflow_exec_transactions_landed_fees_only {}",
                e.transactions_landed_fees_only
            ));
            lines.push(format!(
                "karstflow_exec_transactions_unlanded {}",
                e.transactions_unlanded
            ));
            // Error categories.
            lines.push(format!(
                "karstflow_exec_err_insufficient_funds {}",
                e.err_insufficient_funds
            ));
            lines.push(format!(
                "karstflow_exec_err_invalid_account {}",
                e.err_invalid_account
            ));
            lines.push(format!(
                "karstflow_exec_err_blockhash_not_found {}",
                e.err_blockhash_not_found
            ));
            lines.push(format!(
                "karstflow_exec_err_duplicate_signature {}",
                e.err_duplicate_signature
            ));
            lines.push(format!(
                "karstflow_exec_err_instruction_error {}",
                e.err_instruction_error
            ));
            lines.push(format!(
                "karstflow_exec_err_compute_budget {}",
                e.err_compute_budget
            ));
            lines.push(format!("karstflow_exec_err_other {}", e.err_other));
        }

        if let Some(ref s) = self.shred_network {
            lines.push(format!(
                "karstflow_shred_network_shreds_received {}",
                s.shreds_received
            ));
            lines.push(format!(
                "karstflow_shred_network_shreds_from_turbine {}",
                s.shreds_from_turbine
            ));
            lines.push(format!(
                "karstflow_shred_network_shreds_from_repair {}",
                s.shreds_from_repair
            ));
            lines.push(format!(
                "karstflow_shred_network_shreds_duplicate {}",
                s.shreds_duplicate
            ));
            lines.push(format!(
                "karstflow_shred_network_shreds_signature_invalid {}",
                s.shreds_signature_invalid
            ));
            lines.push(format!(
                "karstflow_shred_network_fec_sets_completed {}",
                s.fec_sets_completed
            ));
            lines.push(format!(
                "karstflow_shred_network_fec_sets_recovered {}",
                s.fec_sets_recovered
            ));
            lines.push(format!(
                "karstflow_shred_network_retransmits_sent {}",
                s.retransmits_sent
            ));
            lines.push(format!(
                "karstflow_shred_network_slots_completed {}",
                s.slots_completed
            ));
        }
        ...
        if let Some(ref f) = self.fec_resolver {
            lines.push(format!(
                "karstflow_fec_resolver_shreds_inserted {}",
                f.shreds_inserted
            ));
            lines.push(format!(
                "karstflow_fec_resolver_sets_completed {}",
                f.sets_completed
            ));
            lines.push(format!(
                "karstflow_fec_resolver_sets_recoverable {}",
                f.sets_recoverable
            ));
            lines.push(format!(
                "karstflow_fec_resolver_sets_spilled {}",
                f.sets_spilled
            ));
            lines.push(format!(
                "karstflow_fec_resolver_duplicates_rejected {}",
                f.duplicates_rejected
            ));
            lines.push(format!(
                "karstflow_fec_resolver_duplicate_shreds_rejected {}",
                f.duplicate_shreds_rejected
            ));
            lines.push(format!(
                "karstflow_fec_resolver_equivocations_detected {}",
                f.equivocations_detected
            ));
            lines.push(format!(
                "karstflow_fec_resolver_chain_breaks {}",
                f.chain_breaks
            ));
        }

        if let Some(ref c) = self.fec_cache {
            lines.push(format!("karstflow_fec_cache_inserts {}", c.inserts));
            lines.push(format!("karstflow_fec_cache_hits {}", c.hits));
            lines.push(format!("karstflow_fec_cache_misses {}", c.misses));
            lines.push(format!("karstflow_fec_cache_evictions {}", c.evictions));
            lines.push(format!(
                "karstflow_fec_cache_cached_count {}",
                c.cached_count
            ));
            lines.push(format!("karstflow_fec_cache_slot_count {}", c.slot_count));
        }

        if let Some(ref g) = self.gossip {
            lines.push(format!(
                "karstflow_gossip_push_messages_sent {}",
                g.push_messages_sent
            ));
            lines.push(format!(
                "karstflow_gossip_push_messages_received {}",
                g.push_messages_received
            ));
            lines.push(format!(
                "karstflow_gossip_pull_requests_sent {}",
                g.pull_requests_sent
            ));
            lines.push(format!(
                "karstflow_gossip_pull_responses_received {}",
                g.pull_responses_received
            ));
            lines.push(format!("karstflow_gossip_pings_sent {}", g.pings_sent));
            lines.push(format!(
                "karstflow_gossip_pongs_received {}",
                g.pongs_received
            ));
            lines.push(format!(
                "karstflow_gossip_nodes_discovered {}",
                g.nodes_discovered
            ));
            lines.push(format!("karstflow_gossip_bytes_sent {}", g.bytes_sent));
            lines.push(format!(
                "karstflow_gossip_bytes_received {}",
                g.bytes_received
            ));
            lines.push(format!("karstflow_gossip_send_errors {}", g.send_errors));
            lines.push(format!(
                "karstflow_gossip_receive_errors {}",
                g.receive_errors
            ));
        }
        ...
        if let Some(ref r) = self.replay {
            lines.push(format!(
                "karstflow_replay_slots_replayed {}",
                r.slots_replayed
            ));
            lines.push(format!("karstflow_replay_slots_dead {}", r.slots_dead));
            lines.push(format!(
                "karstflow_replay_transactions_processed {}",
                r.transactions_processed
            ));
            lines.push(format!(
                "karstflow_replay_transactions_succeeded {}",
                r.transactions_succeeded
            ));
            lines.push(format!(
                "karstflow_replay_transactions_failed {}",
                r.transactions_failed
            ));
            lines.push(format!("karstflow_replay_root_slot {}", r.root_slot));
        }

        if let Some(ref rp) = self.repair {
            lines.push(format!(
                "karstflow_repair_requests_generated {}",
                rp.requests_generated
            ));
            lines.push(format!(
                "karstflow_repair_requests_deduped {}",
                rp.requests_deduped
            ));
            lines.push(format!(
                "karstflow_repair_requests_sent {}",
                rp.requests_sent
            ));
            lines.push(format!(
                "karstflow_repair_responses_accepted {}",
                rp.responses_accepted
            ));
            lines.push(format!(
                "karstflow_repair_responses_duplicate {}",
                rp.responses_duplicate
            ));
            lines.push(format!(
                "karstflow_repair_responses_unknown {}",
                rp.responses_unknown
            ));
            lines.push(format!(
                "karstflow_repair_requests_timed_out {}",
                rp.requests_timed_out
            ));
            lines.push(format!(
                "karstflow_repair_slots_completed {}",
                rp.slots_completed
            ));
            lines.push(format!(
                "karstflow_repair_orphan_requests {}",
                rp.orphan_requests
            ));
        }
        ...
        if let Some(ref c) = self.consensus {
            lines.push(format!(
                "karstflow_consensus_slots_with_votes {}",
                c.total_slots_with_votes
            ));
            lines.push(format!(
                "karstflow_consensus_slots_supermajority {}",
                c.slots_with_supermajority
            ));
            lines.push(format!(
                "karstflow_consensus_slots_propagated {}",
                c.slots_propagated
            ));
            lines.push(format!(
                "karstflow_consensus_slots_duplicate_confirmed {}",
                c.slots_duplicate_confirmed
            ));
            lines.push(format!(
                "karstflow_consensus_slots_super_confirmed {}",
                c.slots_super_confirmed
            ));
            lines.push(format!(
                "karstflow_consensus_total_validators {}",
                c.total_validators
            ));
            lines.push(format!(
                "karstflow_consensus_active_validators {}",
                c.active_validators
            ));
            lines.push(format!("karstflow_consensus_total_stake {}", c.total_stake));
        }

        if let Some(ref b) = self.blockstore {
            lines.push(format!(
                "karstflow_blockstore_shreds_inserted {}",
                b.shreds_inserted
            ));
            lines.push(format!(
                "karstflow_blockstore_slots_completed {}",
                b.slots_completed
            ));
            lines.push(format!("karstflow_blockstore_slots_dead {}", b.slots_dead));
            lines.push(format!(
                "karstflow_blockstore_slots_duplicate {}",
                b.slots_duplicate
            ));
            lines.push(format!(
                "karstflow_blockstore_slots_rooted {}",
                b.slots_rooted
            ));
            lines.push(format!(
                "karstflow_blockstore_slots_purged {}",
                b.slots_purged
            ));
        }
        ...
        if let Some(ref sc) = self.shred_collector {
            lines.push(format!(
                "karstflow_shred_collector_shreds_received {}",
                sc.shreds_received
            ));
            lines.push(format!(
                "karstflow_shred_collector_fec_sets_received {}",
                sc.fec_sets_received
            ));
            lines.push(format!(
                "karstflow_shred_collector_blocks_emitted {}",
                sc.blocks_emitted
            ));
            lines.push(format!(
                "karstflow_shred_collector_slots_evicted_age {}",
                sc.slots_evicted_age
            ));
            lines.push(format!(
                "karstflow_shred_collector_slots_evicted_overflow {}",
                sc.slots_evicted_overflow
            ));
            lines.push(format!(
                "karstflow_shred_collector_assembly_failures {}",
                sc.assembly_failures
            ));
            lines.push(format!(
                "karstflow_shred_collector_downstream_backpressure {}",
                sc.downstream_backpressure
            ));
            lines.push(format!(
                "karstflow_shred_collector_blockstore_writes {}",
                sc.blockstore_writes
            ));
            lines.push(format!(
                "karstflow_shred_collector_blockstore_write_errors {}",
        ...
        if let Some(ref ss) = self.sign_service {
            lines.push(format!(
                "karstflow_sign_service_total_signs {}",
                ss.total_signs
            ));
            lines.push(format!(
                "karstflow_sign_service_shred_signs {}",
                ss.shred_signs
            ));
            lines.push(format!(
                "karstflow_sign_service_vote_signs {}",
                ss.vote_signs
            ));
            lines.push(format!(
                "karstflow_sign_service_gossip_signs {}",
                ss.gossip_signs
            ));
            lines.push(format!(
                "karstflow_sign_service_repair_signs {}",
                ss.repair_signs
            ));
            lines.push(format!(
                "karstflow_sign_service_sign_errors {}",
                ss.sign_errors
            ));
        }
...
mod tests {
    ...
    fn prometheus_lines_include_all_registered_stages() {
        ...
        assert!(text.contains("karstflow_verify_transactions_received 100"));
        assert!(text.contains("karstflow_verify_transactions_verified 95"));
        assert!(text.contains("karstflow_pack_transactions_scheduled 80"));
        assert!(text.contains("karstflow_pack_microblocks_produced 10"));
        assert!(text.contains("karstflow_exec_transactions_executed 75"));
        assert!(text.contains("karstflow_exec_compute_units_consumed 500000"));
        assert!(text.contains("karstflow_exec_compute_units_rebated 0"));
        assert!(text.contains("karstflow_exec_transactions_landed 0"));
        assert!(text.contains("karstflow_exec_transactions_landed_fees_only 0"));
        assert!(text.contains("karstflow_exec_transactions_unlanded 0"));
        assert!(text.contains("karstflow_exec_err_insufficient_funds 0"));
        assert!(text.contains("karstflow_exec_err_instruction_error 0"));
        assert!(text.contains("karstflow_shred_network_shreds_received 1000"));
        assert!(text.contains("karstflow_shred_network_fec_sets_completed 50"));

        // Unregistered stages should NOT appear.
        assert!(!text.contains("karstflow_dedup_"));
        assert!(!text.contains("karstflow_resolv_"));
    }
    ...
    fn fec_resolver_and_cache_snapshots() {
        ...
        assert!(text.contains("karstflow_fec_resolver_shreds_inserted 200"));
        assert!(text.contains("karstflow_fec_resolver_sets_completed 18"));
        assert!(text.contains("karstflow_fec_resolver_sets_recoverable 2"));
        assert!(text.contains("karstflow_fec_resolver_sets_spilled 5"));
        assert!(text.contains("karstflow_fec_resolver_duplicates_rejected 3"));
        assert!(text.contains("karstflow_fec_resolver_duplicate_shreds_rejected 10"));
        assert!(text.contains("karstflow_fec_resolver_equivocations_detected 1"));
        assert!(text.contains("karstflow_fec_cache_hits 500"));
        assert!(text.contains("karstflow_fec_cache_misses 50"));
        assert!(text.contains("karstflow_fec_cache_inserts 100"));
        assert!(text.contains("karstflow_fec_cache_cached_count 90"));
        assert!(text.contains("karstflow_fec_cache_slot_count 45"));
    }
    ...
    fn dedup_snapshot_rendering() {
        ...
        assert!(text.contains("karstflow_dedup_total_checked 200"));
        assert!(text.contains("karstflow_dedup_duplicates_found 30"));
        assert!(text.contains("karstflow_dedup_unique_passed 170"));
    }
    ...
    fn blockstore_stats_prometheus_lines() {
        ...
        assert!(text.contains("karstflow_blockstore_shreds_inserted 500"));
        assert!(text.contains("karstflow_blockstore_slots_completed 10"));
        assert!(text.contains("karstflow_blockstore_slots_dead 2"));
        assert!(text.contains("karstflow_blockstore_slots_duplicate 1"));
        assert!(text.contains("karstflow_blockstore_slots_rooted 8"));
        assert!(text.contains("karstflow_blockstore_slots_purged 3"));
    }
}
