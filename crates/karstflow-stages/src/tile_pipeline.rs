...
use karstflow_mesh::fragment::{ctl_pack, FragmentMeta};
use karstflow_mesh::stem::{InputResult, StemInput, StemOutputConfig, TileStem};
use karstflow_mesh::tile::Tile;
use std::sync::Arc;
        ...
        loop {
            ...
            match self.input.receive(1) {
                InputResult::Ready { meta, payload } => {
                    let source = origin_to_source(karstflow_mesh::fragment::ctl_origin(meta.ctl));
...
/// transaction parser from karstflow-types.
...
fn parse_resolved_transaction(payload: &[u8]) -> ResolvedTransaction {
    ...
    ResolvedTransaction {
        ...
        blockhash,
        priority_fee: 0,
        compute_units: karstflow_constants::execution::MAX_COMPUTE_UNITS,
        is_vote: false,
    }
}
...
impl TransactionPipeline {
    ...
    pub fn with_config(config: PipelineConfig) -> Self {
        use karstflow_constants::dedup::DEFAULT_CACHE_DEPTH;
...
fn resolved_to_packed(payload: Vec<u8>) -> PackedTransaction {
    ...
    match karstflow_types::parse_transaction(&payload) {
        Ok(parsed) => {
            ...
            let is_vote = parsed.message.instructions.iter().any(|ix| {
                ...
                idx < parsed.message.account_keys.len()
                    && parsed.message.account_keys[idx] == karstflow_ids::VOTE_PROGRAM_ID
            });

            // Build instruction views for the cost model.
            let instruction_views: Vec<karstflow_consensus::pack::cost_model::InstructionView<'_>> =
                parsed
                    .message
                    .instructions
                    .iter()
                    .filter_map(|ix| {
                        ...
                        if idx < parsed.message.account_keys.len() {
                            Some(karstflow_consensus::pack::cost_model::InstructionView {
            ...
            let cost = karstflow_consensus::pack::cost_model::compute_transaction_cost(
                &instruction_views,
...
mod tests {
    ...
    fn make_vote_transaction(key: &SigningKey) -> Vec<u8> {
        ...
        message.extend_from_slice(&karstflow_ids::VOTE_PROGRAM_ID.to_bytes()); // account 1: vote program
        message.extend_from_slice(&blockhash); // recent blockhash
    ...
    fn make_transaction_with_compute_budget(key: &SigningKey) -> Vec<u8> {
        ...
        message.extend_from_slice(&karstflow_ids::COMPUTE_BUDGET_PROGRAM_ID.to_bytes()); // account 1
        message.extend_from_slice(&karstflow_ids::SYSTEM_PROGRAM_ID.to_bytes()); // account 2
        ...
        message
            .push(karstflow_constants::compute_budget_program::INSTRUCTION_SET_COMPUTE_UNIT_LIMIT);
        ...
        message
            .push(karstflow_constants::compute_budget_program::INSTRUCTION_SET_COMPUTE_UNIT_PRICE);
