...
use paradencer_core::RuntimeSpec;

pub(super) fn apply_env_overrides(runtime_spec: &mut RuntimeSpec) -> Result<()> {
    if let Ok(mode_value) = std::env::var("PARADENCER_EXEC_MODE") {
        runtime_spec.mode = parse_execution_mode(Some(mode_value))?;
    }
    if let Some(workers) = parse_optional_usize_env("PARADENCER_WORKERS")? {
        runtime_spec.workers = workers;
    }
    if let Some(run_for_seconds) = parse_optional_u64_env("PARADENCER_RUN_SECONDS")? {
        runtime_spec.run_for_seconds = Some(run_for_seconds);
    }
    if let Some(allow_core_sharing) =
        parse_optional_bool_env("PARADENCER_PINNED_ALLOW_CORE_SHARING")?
    {
    ...
    if let Ok(policy_value) = std::env::var("PARADENCER_PINNED_CORE_POLICY") {
        runtime_spec.pinned_core_policy = parse_pinned_core_policy(Some(policy_value))?;
    }
    if let Ok(raw_core_ids) = std::env::var("PARADENCER_PINNED_SERVICE_CORES") {
        runtime_spec.pinned_service_core_ids = Some(parse_pinned_service_cores_env(
            &raw_core_ids,
            "PARADENCER_PINNED_SERVICE_CORES",
        )?);
    }
