...
fn snapshot_catalog_roundtrip_persist_and_load_restores_latest() {
    let mut store = HotStateStore::new();
    let mut catalog = SnapshotCatalog::new();
    let file_path = unique_temp_file("karstflow-snapshot-catalog", "json");
...
fn snapshot_catalog_load_if_exists_returns_none_for_missing_file() {
    let file_path = unique_temp_file("karstflow-no-catalog", "json");
...
fn snapshot_catalog_rejects_unknown_schema_version() {
    let file_path = unique_temp_file("karstflow-invalid-catalog", "json");
    fs::write(
        &file_path,
...
fn snapshot_catalog_migrates_legacy_v0_without_schema_field() {
    let file_path = unique_temp_file("karstflow-legacy-catalog", "json");
    fs::write(
        &file_path,
...
fn snapshot_catalog_migrates_legacy_v0_with_explicit_schema_zero() {
    let file_path = unique_temp_file("karstflow-legacy-catalog-v0", "json");
    fs::write(
        &file_path,
...
fn snapshot_catalog_rejects_non_monotonic_snapshot_fragment_sequence() {
    let file_path = unique_temp_file("karstflow-invalid-order-catalog", "json");
    fs::write(
        &file_path,
...
fn snapshot_catalog_rejects_non_monotonic_snapshot_counters() {
    let file_path = unique_temp_file("karstflow-invalid-counters-catalog", "json");
    fs::write(
        &file_path,
...
fn snapshot_catalog_migrates_legacy_v1_without_checksum_field() {
    let file_path = unique_temp_file("karstflow-legacy-catalog-v1", "json");
    fs::write(
        &file_path,
...
fn snapshot_catalog_rejects_checksum_mismatch() {
    let file_path = unique_temp_file("karstflow-invalid-checksum-catalog", "json");
    fs::write(
        &file_path,
...
fn snapshot_catalog_rejects_header_last_fragment_id_mismatch() {
    let file_path = unique_temp_file("karstflow-invalid-header-last-fragment", "json");
    fs::write(
        &file_path,
...
fn snapshot_catalog_rejects_header_written_count_smaller_than_entries() {
    let file_path = unique_temp_file("karstflow-invalid-header-written-count", "json");
    fs::write(
        &file_path,
