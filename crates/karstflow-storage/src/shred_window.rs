...
use dashmap::DashMap;
use karstflow_crypto::{FecReconstructor, FecResult};
use karstflow_types::shred::{FecSetId, Shred};
...
impl FecSet {
    ...
    fn attempt_reconstruction(&mut self) -> FecResult<Vec<Shred>> {
        ...
        let expected_data = self
            .expected_data_count
            .ok_or(karstflow_crypto::FecError::InvalidParameters { data: 0, coding: 0 })?;
        ...
        for (i, maybe_payload) in result.data_shreds.into_iter().enumerate() {
            if let Some(payload) = maybe_payload {
                ...
                reconstructed.push(Shred::new(
                    karstflow_types::shred::ShredCommonHeader {
                        signature: [0; 64],
                        variant: karstflow_types::shred::SHRED_DATA_FLAG,
                        slot: self.id.slot,
                        index: i as u32,
                        version: 0,
                        fec_set_index: self.id.index,
                    },
                    karstflow_types::shred::ShredVariant::LegacyData(
                        karstflow_types::shred::DataShredHeader {
                            parent_offset: 0,
                            flags: 0,
...
mod tests {
    ...
    fn create_test_coding_shred(
        ...
        Shred::new(
            ShredCommonHeader {
                signature: [0; SIGNATURE_SIZE],
                variant: SHRED_TYPE_LEGACY_CODE | SHRED_LEGACY_CODE_NIBBLE,
                slot,
                index,
                version: 1,
                fec_set_index,
