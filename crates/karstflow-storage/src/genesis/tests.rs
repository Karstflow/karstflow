...
fn default_fee_rate_governor() {
    ...
    assert_eq!(
        ...
        karstflow_constants::economics::LAMPORTS_PER_SIGNATURE
    );
    assert_eq!(
...
fn default_inflation_parameters() {
    let config = GenesisConfig::default_development();
    assert!(
        (config.inflation.initial_rate - karstflow_constants::economics::INFLATION_INITIAL_RATE)
            .abs()
            < f64::EPSILON
    );
    assert!(
        (config.inflation.terminal_rate - karstflow_constants::economics::INFLATION_TERMINAL_RATE)
            .abs()
            < f64::EPSILON
    );
}
